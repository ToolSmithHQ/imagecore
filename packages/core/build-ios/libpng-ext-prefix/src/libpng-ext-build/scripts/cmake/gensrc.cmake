...
set(SRCDIR "/Users/ombhardwaj/Development/projects/aiotool/imagecore/packages/core/third_party/libpng")
set(BINDIR "/Users/ombhardwaj/Development/projects/aiotool/imagecore/packages/core/build-ios/libpng-ext-prefix/src/libpng-ext-build")

set(AWK "")
set(DFA_XTRA "")
set(PNG_PREFIX "")
set(PNGLIB_VERSION "1.8.0")

if(OUTPUT MATCHES "(scripts/pnglibconf/pnglibconf\\.c)\$")
  # Generate "${BINDIR}/scripts/pnglibconf/pnglibconf.c"
  ...
  execute_process(COMMAND "${CMAKE_COMMAND}" -E echo "com ${PNGLIB_VERSION} STANDARD API DEFINITION"
                  COMMAND "${AWK}" -f "${SRCDIR}/scripts/pnglibconf/options.awk"
                          "out=pnglibconf.tf6"
                          "logunsupported=1"
                          "version=search"
                          "${SRCDIR}/pngconf.h" "-"
  ...
  execute_process(COMMAND "${AWK}" -f "${SRCDIR}/scripts/pnglibconf/options.awk"
                  "out=pnglibconf.tf7"
                  "pnglibconf.tf6"
                  WORKING_DIRECTORY "${BINDIR}"
                  RESULT_VARIABLE AWK_FAIL)
  ...
  file(RENAME "pnglibconf.tf7" "${BINDIR}/scripts/pnglibconf/pnglibconf.c")
...
elseif(OUTPUT MATCHES "(pnglibconf\\.h)\$")
  ...
  if(PNG_PREFIX)
    ...
    execute_process(COMMAND "${AWK}" "s==0 && NR>1{print prev}
                             ...
                             END{print prev}"
                             s=0 "pnglibconf.out"
                             s=1 "${BINDIR}/scripts/pnglibconf/prefix.out"
...
elseif(OUTPUT MATCHES "(pnglibconf\\.h\\.prebuilt)\$")
  # Generate pnglibconf.h.prebuilt (fails build)

  message(STATUS "Attempting to build pnglibconf.h.prebuilt")
  ...
  message(STATUS "scripts/pnglibconf/pnglibconf.out to pnglibconf.h.prebuilt")
